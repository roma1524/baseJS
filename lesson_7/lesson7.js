...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        if (score % 4 == 0) {
            antiScore++;
            problemCreateFood();
            document.querySelector('.trabl_span').innerHTML = antiScore;
        }
        score++;
        document.querySelector('.tablo_span').innerHTML = score;
    } else if (unit_classes.includes('food-unit-problem')) finishTheGame(); // Проверка на трабл блок
    return check;
}
...
function problemCreateFood() {  // функция, генерирующая трабл блоки
    ...
    while (foodCreated) { //пока еду не создали
        ...
        if (!food_cell_classes.includes('snake-unit')) {
            ...
            food_cell.setAttribute('class', classes + 'food-unit-problem');
            foodCreated = false;
        }
    }
}
