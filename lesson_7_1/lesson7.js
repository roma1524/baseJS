...
function startGame() {
    ...
    trabl_timer = setInterval(createTrabl, trabl_speed);
}
...
function pathClear(unit) {
    ...
    if (!unit_classes.includes('trabl-unit')) check = true;
...
function createTrabl() {
    var trablCreate = false;

    while (!trablCreate) { //пока еду не создали
        // рандом
        var trabl_x = Math.floor(Math.random() * FIELD_SIZE_X);
        var trabl_y = Math.floor(Math.random() * FIELD_SIZE_Y);

        var trabl_cell = document.getElementsByClassName('cell-' + trabl_y + '-' + trabl_x)[0];
        var trabl_cell_classes = trabl_cell.getAttribute('class').split(' ');

        // проверка на змейку
        if (!trabl_cell_classes.includes('snake-unit') && !trabl_cell_classes.includes('food-unit')) {
            var classes = '';
            for (var i = 0; i < trabl_cell_classes.length; i++) {
                classes += trabl_cell_classes[i] + ' ';
            }

            trabl_cell.setAttribute('class', classes + 'trabl-unit');
            trablCreate = true;
        }
    }
}
